...
io.sockets.on("connection", function (socket) {
    ...
    socket.on('delete_room', function (data) {
        ...
        for (let x of Object.keys(rooms)) {
            if (del_room == x) { // if the room name = x name
                if (rooms[x] == potential_admin) { // if username = x value (username) //idfk
                    ...
                    // if (verify == "Yes") {
                        ...
                        delete rooms[del_room];
                        //room_passwords.delete(del_room);
                        delete room_passwords[del_room];
    ...
    socket.on("ban_user", function(data){ 
        ...
        if (rooms[room] == banning_user) {
            ...
            io.to(banned_id).emit("message_to_client", { message: msg, user: "System Admin"});
        }
        else {
            io.sockets.emit("message_to_client", {message: "You cannot ban this user!", user:"System Admin", room:room});
        }
    });
    ...
    socket.on('kick_user', function(data){
        ...
        if (rooms[room] == kicking_user) {
            if (locations[kicked_user] == room) {
                ...
                io.to(kicked_id).emit("message_to_client", { message: msg, user: "System Admin"});
                io.sockets.emit("display_users", {rooms_keys:Object.keys(rooms), users: Object.keys(users), locations:locations, locations_keys: Object.keys(locations), rooms:rooms});    
            }
            ...
            io.to(users[kicking_user]).emit("message_to_client", {message: "You cannot kick this user!", user:"System Admin", room:room});
        }
    });

    socket.on('transfer_admin', function(data) {
        let old_admin = data["old_admin"];
        let room = data["room"];
        let new_admin = data["new_admin"];
        if (rooms[room] == old_admin) {
            rooms[room] = new_admin;
            console.log("Success!");//log
            console.log("New admin " + new_admin);//log
            io.to(users[new_admin]).emit("message_to_client", {message: "You are the new admin of " + room, user:"System Admin", room:room});
            io.to(users[old_admin]).emit("message_to_client", {message: "You have transferred admin duties of " + room, user:"System Admin", room:room});
        }
        else {
            console.log("Failure.");//log
            io.to(users[old_admin]).emit("message_to_client", {message: "You don't have this authority.", user:"System Admin", room:room});
        }
    });
